...
export default function RRBillEdit({route}){
  ...
  useEffect(() => {
    ...
    if(data !=null){
      ...
      setRR(data.itemdetails.rr)
      setSta_tus(data.itemdetails.sta_tus)
      setDate(data.itemdetails.date)
    }
    getData();
     }, [data]);
  ...
  async function updateRRBill() {
    var axios = require('axios');
  
    var config = {
      method: 'put',
      url: `http://10.0.2.2:8080/rrbill/update/${id}`,
      headers: { 
